frappe.listview_settings['User'] = {
	onload: function (listview) {
		// --- Enable Users ---
		const enableBtn = listview.page.add_inner_button(__('Enable'), function () {
					...
					for (const row of selected) {
						try {
							...
							if (frappe.debug_mode) {
								console.error(`[User Bulk] Failed to enable ${row.name}:`, error);
							}
							frappe.show_alert({
		...
		// Add green color to Enable button
		setTimeout(() => {
			if (enableBtn && enableBtn.length) {
				enableBtn.addClass('btn-success');
			} else {
				listview.page.$inner_toolbar
					.find('button')
					.filter(function () {
						return $(this).text().trim() === __('Enable');
					})
					.first()
		...
		// --- Disable Users ---
		const disableBtn = listview.page.add_inner_button(__('Disable'), function () {
			const selected = listview.get_checked_items();
			if (!selected || !selected.length) {
				frappe.msgprint({
					message: __('Please select at least one User.'),
				...
				try {
					...
					for (const row of selected) {
						...
						try {
							...
							errorCount++;
							if (frappe.debug_mode) {
								console.error(`[User Bulk] Failed to disable ${row.name}:`, error);
							}
							frappe.show_alert({
		...
		setTimeout(() => {
			if (disableBtn && disableBtn.length) {
				disableBtn.addClass('btn-danger');
			} else {
				listview.page.$inner_toolbar
					.find('button')
					.filter(function () {
						return $(this).text().trim() === __('Disable');
					})
					.first()
					.addClass('btn-danger');
			}
		...
		listview.page.add_inner_button(__('Disable All'), function () {
			frappe.confirm(
