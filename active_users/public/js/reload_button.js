...
frappe._active_users.reload = {
	...
	bindClickHandler: function ($container) {
		$container
			.find('[data-au-reload="1"]')
			.off('click')
			.on('click', function () {
				try {
					console.log('[reload_button.js] bindClickHandler (reload_triggered)');
					window.clearCacheAndReload();
				} catch (err) {
					var errMsg =
						err && err.message ? err.message.substring(0, 50) : 'reload_error';
					console.log('[reload_button.js] bindClickHandler (' + errMsg + ')');
				}
			});
	},
